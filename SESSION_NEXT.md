...
Recent work:
- Hardened Bambu queue starts against H2D ghost-print states. Live H2D showed `state=printing` and queue job `#60` was marked `printing`, but the user's photo and telemetry showed both nozzle targets and bed target were still `0`, progress was `0%`, and layer remained stale at `100/100`. `_advance_queue_specific()` now waits up to 90s after the Bambu MQTT start command for physical proof of start (heater target > 0, progress > 0, or layer_current < layer_total) before calling `queue_set_started()`. If the printer accepts the MQTT command but never heats/progresses, the queue row is failed with a clear "accepted start command but did not begin heating" error and logs `queue_bambu_start_unconfirmed`. Backend restart required. No AMS mapping code was touched.
