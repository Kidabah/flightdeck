...
- Refresh cachebust currently: app.js?v=478 / style.css?v=376 / demo-runtime.js?v=8

Recent work:
- Hardened H2D AMS mapping so impossible nozzle/AMS combinations are blocked before MQTT instead of becoming printer alarm `1800-8012`. Live evidence showed the retried `Revised Wheel_PLA_2h38m.gcode.3mf` now carries `filament_colors` with `nozzle: 1` (left nozzle) while the matching grey PLA is loaded in AMS HT `128` (right nozzle). The Bambu queue and relay mappers now require a colour/material match on the same nozzle path whenever the 3MF specifies a nozzle; if the matching tray is on the other path, Flightdeck raises a clear "move filament or re-slice" error and never sends `project_file` to the printer. Queue preflight now shows the same H2D nozzle/AMS block before dispatch. Backend restart required.
