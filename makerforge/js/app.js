...
import { ensureHoodieStubbyMesh, drapeArtOntoHoodieChest } from "./hoodie-stubby.js?v=579";
import {
  MAX_ACCENT_BANDS,
  newAccentBand,
    ...
    if (separateArt) {
      ...
      if (traceData?.multiColour && traceData.colorLayers?.length) {
        ...
        for (const cp of colourParts || []) {
          if (state.shape === "stubbyHolder") drapeArtOntoHoodieChest(cp.mesh, exportCache.chestField);
        ...
        if (state.shape === "stubbyHolder") drapeArtOntoHoodieChest(artMesh, exportCache.chestField);
